...
declare -r tmp_file="/tmp/tmp_file"
declare -r tmp_file2="/tmp/tmp_file2"
declare -r tmp_file3="/tmp/tmp_file3"
...
function beginEnumeration(){
	...
	if rpcclient -U "${username}%${password}" $host_ip -c "enumdomusers" > /dev/null 2>&1; then
			...
			echo -e "\n${redColour}Access Denied $host_ip${endColour}"
			tput cnorm; exit 0
	fi
}
